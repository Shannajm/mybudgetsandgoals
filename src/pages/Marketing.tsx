...
import { ArrowRight, PiggyBank, Wallet, Calendar, TrendingUp, Target, BarChart3, CreditCard } from 'lucide-react';
...
const ScreenshotCard: React.FC<{ title: string; desc: string }> = ({ title, desc }) => (
    ...
    <div className="relative min-h-screen bg-gray-50 dark:bg-gray-950 overflow-x-hidden">
      ...
      <header className="relative z-10 mx-auto max-w-6xl px-6 py-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-blue-600 to-fuchsia-600 flex items-center justify-center shadow-sm">
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-6 pb-16">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Feature icon={<PiggyBank className="h-5 w-5" />} title="Fixed Savings" desc="Partner plans and fixed deposits with multi‑period contributions." />
        ...
        <section className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-4">
          <ScreenshotCard title="Accounts" desc="See balances by currency and account type." />
          <ScreenshotCard title="Bills & Reminders" desc="Pay with conversion and keep schedules on track." />
          <ScreenshotCard title="Fixed Savings" desc="Contribute multiple weeks at once with clear progress." />
        </section>
